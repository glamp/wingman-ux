...
export class ProxyHandler {
  private logger = createLogger('Wingman:ProxyHandler');
  ...
  async handleRequest(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const developerWs = this.connectionManager.getDeveloperConnection(sessionId);
      if (!developerWs) {
        res.status(502).json({
      ...
      try {
        ...
        } else if (error.message === 'Developer not connected') {
          res.status(502).json({
  ...
  async handleWebSocketUpgrade(sessionId: string, ws: any, req: any): Promise<void> {
    ...
    // Forward WebSocket messages between PM and developer
